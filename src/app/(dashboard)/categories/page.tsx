    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={seeding}
            className="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all active:scale-[0.98] disabled:opacity-50"
          ...
          <button
            ...
            className="px-3.5 py-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold rounded-xl text-xs flex items-center gap-1.5 transition-all shadow-md shadow-emerald-500/20 active:scale-[0.98]"
          >
      ...
      {loading ? (
        ...
        <div className="p-12 text-center bg-slate-900/50 border border-dashed border-slate-800 rounded-2xl space-y-3">
          ...
          <div className="text-sm font-semibold text-slate-200">No {activeTab.toLowerCase()} categories found</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {roots.map((parent) => {
            ...
            return (
              <div
                key={parent.id}
                className="p-4 bg-slate-900 border border-slate-800 rounded-2xl space-y-3 flex flex-col justify-between hover:border-slate-700/80 transition-all"
              >
                <div>
                  {/* Category Header */}
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      <div
                        className="w-9 h-9 rounded-xl flex items-center justify-center text-slate-950 shadow-sm"
                        style={{ backgroundColor: parent.color || '#10B981' }}
                      >
                        <CategoryIcon name={parent.icon} className="w-4 h-4 text-slate-950 stroke-[2.5]" />
                      </div>
                      <div>
                        <div className="text-sm font-bold text-white flex items-center gap-2">
                          <span>{parent.name}</span>
                          <span className="text-[10px] px-2 py-0.5 bg-slate-800 text-slate-400 rounded-full font-normal">
                            {children.length} sub
                          </span>
                        </div>
                    ...
                    {children.length === 0 ? (
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {children.map((sub) => (
                          <span
                            key={sub.id}
                            className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-800/80 hover:bg-slate-800 border border-slate-700/60 text-xs text-slate-300 group transition-colors"
                          >
                            <span
